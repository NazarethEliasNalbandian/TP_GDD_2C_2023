...
AS
BEGIN
	INSERT INTO SQLSQUAD.BI_Ambientes(AMBIENTES_ID,  AMBIENTES_NOMBRE)
    SELECT C.CANT_AMBIENTES_ID, C.CANT_AMBIENTES_NOMBRE
    FROM SQLSQUAD.cant_ambientes C
END
GO

CREATE PROCEDURE MigrarDatosAnuncio
AS
BEGIN
	 INSERT INTO SQLSQUAD.BI_Anuncio(ANUNCIO_TIEMPO_ID, ANUNCIO_AMBIENTES_ID, ANUNCIO_UBICACION_ID, ANUNCIO_TIPO_OPERACION_ID, ANUNCIO_SUCURSAL_CODIGO, ANUNCIO_TIPO_MONEDA_ID, ANUNCIO_RANGO_M2_ID, ANUNCIO_TIPO_INMUEBLE_ID, ANUNCIO_RANGO_ETARIO_ID)
END
...
AS
SELECT DISTINCT TIEMPO_AÑO, TIEMPO_CUATRIMESTRE, TIPO_OPERACION_NOMBRE, UBICACION_BARRIO, AMBIENTES_NOMBRE, AVG(TIEMPO_DIA) FROM SQLSQUAD.BI_Anuncio
...
AS
SELECT DISTINCT TIEMPO_AÑO, TIEMPO_CUATRIMESTRE, TIPO_OPERACION_NOMBRE, TIPO_INMUEBLE_NOMBRE, MAYOR_A_100, ENTRE_75_Y_100, ENTRE_55_75, ENTRE_35_Y_55, MENOR_A_35 , SQLSQUAD.SignoMoneda(TIPO_MONEDA_NOMBRE) + ' '+ AVG(ANUNCIO_COSTO_ANUNCIO), TIPO_MONEDA_NOMBRE FROM SQLSQUAD.BI_Anuncio
JOIN SQLSQUAD.BI_Tipo_Operacion ON TIPO_OPERACION_ID = ANUNCIO_TIPO_OPERACION_ID
...
JOIN SQLSQUAD.BI_Tiempo ON TIEMPO_ID = ANUNCIO_TIEMPO_ID
JOIN SQLSQUAD.BI_Moneda ON TIPO_MONEDA_ID = ANUNCIO_TIPO_MONEDA_ID
...
JOIN SQLSQUAD.BI_Pago ON PAGO_ID = ALQUILER_PAGO_ID
JOIN SQLSQUAD.BI_Tiempo ON TIEMPO_ID = ALQUILER_TIEMPO_ID
JOIN SQLSQUAD.BI_Estado ON ESTADO_ID = ALQUILER_ESTADO_ID
