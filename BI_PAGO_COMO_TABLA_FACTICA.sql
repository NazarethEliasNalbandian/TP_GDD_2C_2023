...
CREATE TABLE SQLSQUAD.BI_Pago_alquiler(
    ...
    CONSTRAINT PK_22 PRIMARY KEY NONCLUSTERED (BI_PAGO_ALQUILER_TIEMPO_ID, BI_PAGO_ALQUILER_ESTADO_ALQUILER_ID, BI_PAGO_ALQUILER_FECHA_PAGO_ID)
)
GO
...
CREATE TABLE SQLSQUAD.BI_Venta_inmueble(
    ...
    VENTA_SUPERFICIETOTAL         numeric(18, 2)    NULL,
...
ALTER TABLE SQLSQUAD.BI_Pago_alquiler ADD CONSTRAINT RefBI_Tiempo311 
    FOREIGN KEY (BI_PAGO_ALQUILER_TIEMPO_ID)
    REFERENCES SQLSQUAD.BI_Tiempo(BI_TIEMPO_ID)
GO

ALTER TABLE SQLSQUAD.BI_Pago_alquiler ADD CONSTRAINT RefBI_Estado_alquiler321 
    FOREIGN KEY (BI_PAGO_ALQUILER_ESTADO_ALQUILER_ID)
    REFERENCES SQLSQUAD.BI_Estado_alquiler(BI_ESTADO_ALQUILER_ID)
GO

ALTER TABLE SQLSQUAD.BI_Pago_alquiler ADD CONSTRAINT RefBI_Fecha_Pago341 
...
GO
ALTER FUNCTION SQLSQUAD.cumplioPago(@fecha_pago datetime, @fecha_vencimiento datetime)
RETURNS int
BEGIN
...
AS 
BEGIN
	 INSERT INTO SQLSQUAD.BI_Pago_alquiler(BI_PAGO_ALQUILER_TIEMPO_ID, BI_PAGO_ALQUILER_ESTADO_ALQUILER_ID, BI_PAGO_ALQUILER_FECHA_PAGO_ID, BI_PAGO_ALQUILER_IMPORTE)
	 SELECT BI_TIEMPO_ID, ALQUILER_ESTADO_ALQUILER, BI_FECHA_PAGO_ID, SUM(PAGO_ALQUILER_IMPORTE) AS [Importe]
	 FROM SQLSQUAD.pago_alquiler
	 ...
	 GROUP BY BI_TIEMPO_ID, ALQUILER_ESTADO_ALQUILER, BI_FECHA_PAGO_ID
END
GO
...
AS
BEGIN
	INSERT INTO SQLSQUAD.BI_Alquiler(BI_ALQUILER_TIEMPO_ID, BI_ALQUILER_UBICACION_ID, BI_ALQUILER_TIPO_OPERACION_ID, BI_ALQUILER_SUCURSAL_CODIGO, BI_ALQUILER_TIPO_INMUEBLE_ID, BI_ALQUILER_RANGO_ETARIO_ID, BI_ALQUILER_ESTADO_ALQUILER_ID, BI_ALQUILER_COMISION)
	SELECT BI_TIEMPO_ID, BI_UBICACION_ID, ANUNCIO_TIPO_OPERACION_ID, AGENTE_SUCURSAL_CODIGO, INMUEBLE_TIPO_INMUEBLE_ID, SQLSQUAD.calcular_Rango_etario(SQLSQUAD.calcularEdad(AGENTE_FECHA_NAC)) AS [Rango Etario], ALQUILER_ESTADO_ALQUILER, SUM(ALQUILER_COMISION) AS [Comision]
	...
	JOIN SQLSQUAD.BI_Ubicacion  ON (SELECT BI_UBICACION_ID 
										...
										WHERE BARRIO_NOMBRE	= BI_UBICACION_BARRIO
											AND LOCALIDAD_NOMBRE	  = BI_UBICACION_LOCALIDAD
											AND PROVINCIA_NOMBRE	  = BI_UBICACION_PROVINCIA)					 = BI_UBICACION_ID
	GROUP BY BI_TIEMPO_ID, BI_UBICACION_ID, ANUNCIO_TIPO_OPERACION_ID, AGENTE_SUCURSAL_CODIGO, INMUEBLE_TIPO_INMUEBLE_ID, SQLSQUAD.calcular_Rango_etario(SQLSQUAD.calcularEdad(AGENTE_FECHA_NAC)), ALQUILER_ESTADO_ALQUILER
END
GO
...
BEGIN
	...
	SELECT BI_TIEMPO_ID, BI_UBICACION_ID, ANUNCIO_TIPO_OPERACION_ID, AGENTE_SUCURSAL_CODIGO, VENTA_MONEDA_ID, SQLSQUAD.calcular_Rango_m2(INMUEBLE_SUPERFICIETOTAL) AS [Rango m2], INMUEBLE_TIPO_INMUEBLE_ID, SUM(VENTA_PRECIO_VENTA) AS [Precio venta], SUM(VENTA_COMISION) AS [Comision]
	FROM SQLSQUAD.venta
...
SELECT T1.BI_TIEMPO_AÑO, T1.BI_TIEMPO_MES, 100-(SELECT COUNT(*) FROM SQLSQUAD.BI_Pago_alquiler PA2
												...
												AND BI_FECHA_PAGO_ID = BI_PAGO_ALQUILER_FECHA_PAGO_ID AND T1.BI_TIEMPO_AÑO = T2.BI_TIEMPO_AÑO AND T1.BI_TIEMPO_MES = T2.BI_TIEMPO_MES)* 100 / (COUNT(*)) AS [Porcentaje de incumplimiento]
FROM SQLSQUAD.BI_Pago_alquiler PA1
JOIN SQLSQUAD.BI_Tiempo T1        ON T1.BI_TIEMPO_ID     = PA1.BI_PAGO_ALQUILER_TIEMPO_ID
JOIN SQLSQUAD.BI_Fecha_Pago FP1 ON FP1.BI_FECHA_PAGO_ID = PA1.BI_PAGO_ALQUILER_FECHA_PAGO_ID
