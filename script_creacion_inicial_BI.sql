...
ALTER PROCEDURE MigrarDatosBI_Pago_Alquiler
AS 
BEGIN
	 ...
	 SELECT BI_TIEMPO_ID, ALQUILER_ESTADO_ALQUILER, BI_FECHA_PAGO_ID,ANUNCIO_TIPO_OPERACION_ID, 1, SUCURSAL_CODIGO, SUM(PAGO_ALQUILER_IMPORTE) AS [Importe]
	 FROM SQLSQUAD.pago_alquiler
	 ...
	 GROUP BY BI_TIEMPO_ID, ALQUILER_ESTADO_ALQUILER, BI_FECHA_PAGO_ID,ANUNCIO_TIPO_OPERACION_ID, SUCURSAL_CODIGO
END
GO
...
BEGIN
	...
	SELECT BI_TIEMPO_ID, BI_UBICACION_ID, ANUNCIO_TIPO_OPERACION_ID, AGENTE_SUCURSAL_CODIGO, INMUEBLE_TIPO_INMUEBLE_ID, SQLSQUAD.calcular_Rango_etario(SQLSQUAD.calcularEdad(AGENTE_FECHA_NAC)) AS [Rango Etario], ALQUILER_ESTADO_ALQUILER, AVG(ALQUILER_COMISION) AS [Comision], COUNT(*) AS [Cantidad de alquileres]
...
AS
SELECT BI_TIEMPO_AÑO, BI_TIEMPO_MES, AVG(BI_PAGO_ALQUILER_IMPORTE*100/
									  (SELECT PA2.BI_PAGO_ALQUILER_IMPORTE FROM SQLSQUAD.BI_Pago_alquiler PA2
										...
										WHERE T2.BI_TIEMPO_MES = MONTH(GETDATE()) AND T2.BI_TIEMPO_AÑO = YEAR(GETDATE()))) 
FROM SQLSQUAD.BI_Pago_alquiler PA1
...
WHERE BI_PAGO_ALQUILER_IMPORTE < (SELECT PA2.BI_PAGO_ALQUILER_IMPORTE FROM SQLSQUAD.BI_Pago_alquiler PA2
										JOIN SQLSQUAD.BI_Tiempo T2 ON BI_TIEMPO_ID = BI_PAGO_ALQUILER_TIEMPO_ID
										JOIN SQLSQUAD.BI_Estado_Alquiler EA2 ON BI_ESTADO_ALQUILER_ID = BI_PAGO_ALQUILER_ESTADO_ALQUILER_ID
										WHERE T2.BI_TIEMPO_MES = MONTH(GETDATE()) AND T2.BI_TIEMPO_AÑO = YEAR(GETDATE()))
