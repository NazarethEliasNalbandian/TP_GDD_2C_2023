...
BEGIN
    ...
    D.DIRECCION_ID
	FROM gd_esquema.Maestra 
	JOIN SQLSQUAD.tipo_inmueble AS TI ON TI.TIPO_INMUEBLE_NOMBRE = INMUEBLE_TIPO_INMUEBLE
	JOIN SQLSQUAD.disposicion AS DI ON DI.DISPOSICION_NOMBRE = INMUEBLE_DISPOSICION
	JOIN SQLSQUAD.orientacion AS ORI ON ORI.ORIENTACION_NOMBRE = INMUEBLE_ORIENTACION
	JOIN SQLSQUAD.cant_ambientes AS CA ON CA.CANT_AMBIENTES_NOMBRE = INMUEBLE_CANT_AMBIENTES
	JOIN SQLSQUAD.estado_inmueble AS EI ON EI.ESTADO_INMUEBLE_NOMBRE = INMUEBLE_ESTADO
	JOIN SQLSQUAD.direccion AS D ON SQLSQUAD.DireccionCompleta(D.DIRECCION_CALLE, D.DIRECCION_NUMERO_CALLE) = INMUEBLE_DIRECCION
	WHERE INMUEBLE_CODIGO IS NOT NULL
			AND D.DIRECCION_BARRIO_ID = (SELECT BARRIO_ID FROM SQLSQUAD.barrio WHERE BARRIO_NOMBRE = INMUEBLE_BARRIO
			AND BARRIO_LOCALIDAD_ID = (SELECT LOCALIDAD_ID FROM SQLSQUAD.localidad WHERE LOCALIDAD_NOMBRE = INMUEBLE_LOCALIDAD
			AND LOCALIDAD_PROVINCIA_ID = (SELECT PROVINCIA_ID FROM SQLSQUAD.provincia WHERE PROVINCIA_NOMBRE = INMUEBLE_PROVINCIA)))																																																																												WHERE LOCALIDAD_PROVINCIA_ID = PROVINCIA_ID))))
END
GO
