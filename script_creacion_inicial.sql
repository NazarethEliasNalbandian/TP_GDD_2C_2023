 ...
 * Author :       Nachi
 *
 * Date Created : Thursday, October 05, 2023 01:44:22
...
BEGIN
    ...
    SELECT DISTINCT
        SQLSQUAD.NombreCalle(INMUEBLE_DIRECCION), SQLSQUAD.NumeroCalle(INMUEBLE_DIRECCION), BARRIO_ID
    FROM gd_esquema.Maestra
...
BEGIN
    ...
    SELECT DISTINCT
		...
		DIRECCION_ID
    FROM gd_esquema.Maestra 
	JOIN SQLSQUAD.direccion ON DIRECCION_ID = (SELECT DIRECCION_ID FROM SQLSQUAD.direccion												WHERE SQLSQUAD.NombreCalle(SUCURSAL_DIRECCION) +  SQLSQUAD.NumeroCalle(SUCURSAL_DIRECCION) = SUCURSAL_DIRECCION)
END
GO
...
BEGIN
    ...
    SELECT DISTINCT
		INMUEBLE_CODIGO, INMUEBLE_NOMBRE, INMUEBLE_EXPESAS, INMUEBLE_ANTIGUEDAD, INMUEBLE_DESCRIPCION, INMUEBLE_SUPERFICIETOTAL,TIPO_INMUEBLE_ID, DISPOSICION_ID, ORIENTACION_ID, CANT_AMBIENTES_ID, ESTADO_INMUEBLE_ID, DIRECCION_ID
    FROM gd_esquema.Maestra
	JOIN SQLSQUAD.tipo_inmueble ON TIPO_INMUEBLE_NOMBRE = INMUEBLE_TIPO_INMUEBLE
	JOIN SQLSQUAD.disposicion ON DISPOSICION_NOMBRE = INMUEBLE_DISPOSICION
	JOIN SQLSQUAD.orientacion ON ORIENTACION_NOMBRE = INMUEBLE_ORIENTACION
	JOIN SQLSQUAD.cant_ambientes ON CANT_AMBIENTES_NOMBRE = INMUEBLE_CANT_AMBIENTES
	JOIN SQLSQUAD.estado_inmueble ON ESTADO_INMUEBLE_NOMBRE = INMUEBLE_ESTADO
	JOIN SQLSQUAD.direccion ON SQLSQUAD.DireccionCompleta(DIRECCION_CALLE,DIRECCION_NUMERO_CALLE) = INMUEBLE_DIRECCION
END
GO
